...
<html>
<head>
	<title>PC Assignments || Checkout System</title>
	<meta charset="utf-8">
	...
	<div class="contianer"><!--Page Wrapper-->
		...
		<div class="container" style="margin-top:125px;"><!--Page Contents-->			
			<div><!--Section left-->
				...
				<h4 class="text-center" style="color:whitesmoke; background-color:grey;"><b>PC ASSIGNMENTS</b></h4><br>

					<?php
						$query_table = mysqli_query($conn,"SELECT * FROM asset_tracker.pc_assignment");
				...
				?>
						...
						?>
							...
							<td> <?php 
									echo '<button name="del" class="btn btn-danger">Unassign</button>';									
								 ?>
							</td>
								...
								if(isset($_POST['del']))
							{
								...
								$query_table = mysqli_query($conn,"DELETE FROM asset_tracker.pc_assignment WHERE id='$id'");
								if($query_table)
								{
									echo "<script>alert('Unassigned Successfully!');</script>";
									echo "<script>window.location.href='view_pc_assignment.php';</script>";
								}
							}
